...
exports.studentreport = async (req, res) => {
    try {
        ...
        let limit = 20;
        let totalpage = Math.ceil(200 / limit);
        if (l < 1 || isNaN(l)) {
            l = 1;
        }
        if (l > totalpage) {
        ...
        group by id order by ${column} ${order}  limit ${limit} offset ${offset}`;
...
exports.studentresult = async (req, res) => {
    try {
        ...
        let querylimit = 60;
        let offset = (Number(l) - 1) * querylimit;
        ...
        group by s.id,e.exam_type limit ${querylimit} offset ${offset}`;
